  ...
  if (fetchError) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="bg-white p-12 rounded-3xl shadow-xl border border-red-100 max-w-lg text-center">
          <span className="text-5xl mb-4 block">⚠️</span>
          <h2 className="text-2xl font-bold text-red-600 mb-3">Database Connection Error</h2>
          <p className="text-gray-500 text-sm font-mono bg-gray-50 p-4 rounded-xl">{fetchError}</p>
          <Link href="/" className="mt-8 inline-block px-6 py-3 bg-[var(--primary-green)] text-white font-bold rounded-xl">Return Home</Link>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-16">
      <div className="container mx-auto px-6 max-w-7xl">
        ...
        {/* Country Cards — rendered on the server, always visible */}
        ...
        {/* Interactive university search — client island */}
        <DestinationsClient universities={universities} />
      </div>
    </div>
  );
}
