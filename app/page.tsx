...
export default async function HomePage() {
  ...
  try {
    countries = await prisma.country.findMany({ include: { costEstimates: true } });
  ...
  return (
    <div>
      ...
      {/* Error Banner */}
      {fetchError && (
        <div className="bg-red-50 border-l-4 border-red-500 p-6 mx-6 mt-8 rounded-xl">
          <p className="font-bold text-red-700">⚠️ Database Error: {fetchError}</p>
          <p className="text-red-500 text-sm mt-1">The AI Matcher and destination grid could not load data. Please check Vercel environment variables.</p>
        </div>
      )}

      {/* AI Matching Engine (interactive client island) */}
      {!fetchError && (
        <section className="py-12 bg-gray-50 px-6">
          <HomeClient universities={universities} />
        </section>
      )}

      {/* Destination Grid — server rendered, always visible */}
      <section className="py-24 container mx-auto px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            ...
            return (
              <div key={country.id} className="bg-white rounded-2xl shadow-sm border border-gray-100 hover:shadow-2xl transition-all duration-300 overflow-hidden group flex flex-col">
                <div className="h-48 bg-gray-200 relative flex items-center justify-center overflow-hidden">
                  <div className="absolute inset-0 bg-[var(--primary-green)] opacity-10 group-hover:opacity-30 transition-opacity"></div>
                  <span className="text-6xl relative z-10 group-hover:scale-125 transition-transform duration-500">🌍</span>
                </div>
                <div className="p-8 flex flex-col flex-grow">
                  ...
                  {minTuition ? (
                    <div className="flex items-center justify-center text-sm font-bold text-[var(--primary-green)] mb-6 bg-green-50/80 p-4 rounded-xl">
                      <span className="mr-2 text-xl">💰</span>Est. Tuition: ${minTuition.toLocaleString()}/yr
                    </div>
                  ) : (
                    <div className="mb-6 text-sm text-gray-400 italic p-4 text-center">Tuition data pending</div>
                  )}
                  <Link href={`/destinations/${country.slug}`} className="block w-full text-center py-3 border-2 border-[var(--primary-green)] text-[var(--primary-green)] font-bold rounded-xl hover:bg-[var(--primary-green)] hover:text-white transition-all shadow-sm">
                    View Guide
                  </Link>
                </div>
              </div>
          ...
          {countries.length === 0 && !fetchError && (
            <div className="col-span-full text-center text-gray-500 italic p-8">No destination data found. Please check your database seed.</div>
          )}
        </div>
      </section>
    </div>
